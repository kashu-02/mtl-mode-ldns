	...
	for (current_service = 0;
		...
		if (ldns_get_bit(&(ldns_rdf_data(rdf)[1]), current_service)) {
			...
			if (service && service->s_name) {
				...
				ldns_buffer_printf(output, "%lu ", current_service);
			}
...
#if defined(PQC_ALGO_SLH_DSA_MTL_SHA2) || defined (PQC_ALGO_SLH_DSA_MTL_SHAKE)
static ldns_status
ldns_mtl_key2buffer_str(ldns_buffer *output, const ldns_key *k)
{
	...
	if((k == NULL) || (output == NULL)) {
        return LDNS_STATUS_ERR;
	}
    priv_key = ldns_key_external_key(k);
    if(priv_key == NULL) {
        return LDNS_STATUS_ERR;
    }
    ...
    // Write underlying signature secret key
    ldns_buffer_write_u32(key_buffer, priv_key->sk_len);
    ldns_buffer_write(key_buffer, priv_key->sk, priv_key->sk_len);
    // Write underlying signature public key
    ldns_buffer_write_u32(key_buffer, priv_key->pk_len);
    ldns_buffer_write(key_buffer, priv_key->pk, priv_key->pk_len);
    ...
    // Write the mtl seed
    ldns_buffer_write_u32(key_buffer, mtl_ctx->seed.length);
    ldns_buffer_write(key_buffer, mtl_ctx->seed.seed, mtl_ctx->seed.length);
    // Write the algorithm parameters values
	SPX_PARAMS* mtl_params = (SPX_PARAMS*)mtl_ctx->sig_params;
	if(mtl_params == NULL) {
        ldns_buffer_free(key_buffer);
        return LDNS_STATUS_NO_DATA;		
	}
	// Seed
	ldns_buffer_write_u16(key_buffer, mtl_params->pk_seed.length);
	ldns_buffer_write(key_buffer, mtl_params->pk_seed.seed, mtl_params->pk_seed.length);
	// PK Root
	ldns_buffer_write_u16(key_buffer, mtl_params->pk_root.length);
	ldns_buffer_write(key_buffer, mtl_params->pk_root.key, mtl_params->pk_root.length);
	// SPK_PRF
	ldns_buffer_write_u16(key_buffer, mtl_params->prf.length);
	ldns_buffer_write(key_buffer, mtl_params->prf.data, mtl_params->prf.length);
	// Robust paramter - this has been deprecated in latest MTL mode
	ldns_buffer_write_u8(key_buffer, mtl_params->robust);

    // Write the mtl context string - if any
    if(mtl_ctx->ctx_str != NULL) {
        ldns_buffer_write_u16(key_buffer, strlen(mtl_ctx->ctx_str));
        ldns_buffer_write_string(key_buffer, mtl_ctx->ctx_str);
    } else {
        ldns_buffer_write_u16(key_buffer, 0);
    }
    // Write the mtl sid
    ldns_buffer_write_u32(key_buffer, mtl_ctx->sid.length);
    ldns_buffer_write(key_buffer, mtl_ctx->sid.id, mtl_ctx->sid.length);
    // Write the leaf count
    ldns_buffer_write_u32(key_buffer, mtl_ctx->nodes.leaf_count);
    // Write the hash size
    ldns_buffer_write_u16(key_buffer, mtl_ctx->nodes.hash_size);

    // Write the tree page size
    ldns_buffer_write_u32(key_buffer, mtl_ctx->nodes.tree_page_size);
    // Get the tree pages
    ...
    // Write the tree page count
    ldns_buffer_write_u32(key_buffer, pages);
	// Write the pages
    for (index = 0; index < pages; index++) {
        ldns_buffer_write(key_buffer, 
                    mtl_ctx->nodes.tree_pages[index],
                    mtl_ctx->nodes.tree_page_size);
	}

    // Write the randomizer pages
    ...
    ldns_buffer_printf(output, "Key: ");
    b64_buffer =  ldns_rdf_new_frm_data(LDNS_RDF_TYPE_B64, 
										   ldns_buffer_position(key_buffer),
                                           ldns_buffer_begin(key_buffer));
	if(b64_buffer != NULL) {
		status = ldns_rdf2buffer_str(output, b64_buffer);
		ldns_buffer_printf(output, "\n");
		ldns_rdf_deep_free(b64_buffer);
	} else {
...
ldns_oqs_key2buffer_str(ldns_buffer *output, const ldns_key *k)
{
    ...
    key_buffer = ldns_buffer_new(LDNS_MAX_PACKETLEN);
    oqs_key* priv_key = ldns_key_external_key(k);
    if(priv_key == NULL) {
        return LDNS_STATUS_ERR;
    }
	...
	if (ldns_buffer_status_ok(output)) {
		...
		switch(ldns_key_algorithm(k)) {
            ...
            case LDNS_SIGN_FL_DSA_512:
                ldns_buffer_printf(output, "Private-key-format: v1.2\n");
                ldns_buffer_printf(output, "Algorithm: %d (LDNS_SIGN_FL_DSA_512)\n", LDNS_SIGN_FL_DSA_512);
			...
			case LDNS_SIGN_SLH_DSA_MTL_SHA2_128s:
				ldns_buffer_printf(output, "Private-key-format: v1.2\n");
				ldns_buffer_printf(output, "Algorithm: %d (SLH_DSA_MTL_SHA2_128s)\n", LDNS_SIGN_SLH_DSA_MTL_SHA2_128s);
