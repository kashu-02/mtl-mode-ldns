...
/**
 * Creates and initalizes the global ladder cache
 *
 * \param[in] max elements in include in the cache (LIFO)
 */
void
ldns_dnssec_mtl_ladder_cache_init(uint32_t cache_size)
{
    if(ladder_cache == NULL) {
        ladder_cache = calloc(sizeof(ldns_dnssec_mtl_ladder_cache),1);
        if(ladder_cache == NULL) {
            LOG_ERROR("Unable to allocate the ladder cache");
        }
        ladder_cache->max_entries = cache_size;
        ladder_cache->index = 0;
        ladder_cache->cache = calloc(cache_size, sizeof(LADDER));
        if(ladder_cache->cache == NULL) {
            LOG_ERROR("Unable to allocate the ladder cache");
            free(ladder_cache);
            ladder_cache = NULL;
        }
...
/**
 * Frees the global ladder cache
 */
void
ldns_dnssec_mtl_ladder_cache_free(void) {
    if(ladder_cache != NULL) {
        for(uint32_t i=0; i<ladder_cache->max_entries; i++) {
            if(ladder_cache->cache[i] != NULL) {
                mtl_ladder_free(ladder_cache->cache[i]);
            }
        }
        free(ladder_cache->cache);
        free(ladder_cache);
    }
}

/**
 * Insert a ladder into the cache
 *
 * \param[in] max elements in include in the cache (LIFO)
 */
void
ldns_dnssec_mtl_ladder_cache_insert(LADDER* ladder) {
    if(ladder_cache == NULL) {
        LOG_ERROR("Ladder Cache is NULL (not initalized)");
        return;
    }
    
    // If there is another record here, then free it
    if(ladder_cache->cache[ladder_cache->index] != NULL) {
        mtl_ladder_free(ladder_cache->cache[ladder_cache->index]);
    }
    ladder_cache->cache[ladder_cache->index] = ladder;
    ladder_cache->index++;
    if(ladder_cache->index >= ladder_cache->max_entries) {
        ladder_cache->index = 0;
    }
}

/**
 * Find the best rung in cache for an auth path
 *
 * \param[in] auth path that will be verified
 * \return Rung to use to verify the auth path
 */
RUNG* land_dnssec_mtl_ladder_find_rung(AUTHPATH* path) {
    RUNG* target = NULL;
    uint32_t target_span = 0;

    for(uint16_t l=0; l<ladder_cache->max_entries; l++)
    {
        if(ladder_cache->cache[l] != NULL)
        {
            if((ladder_cache->cache[l]->flags == path->flags) && 
                (ladder_cache->cache[l]->sid.length == path->sid.length) && 
                (memcmp(ladder_cache->cache[l]->sid.id, path->sid.id, ladder_cache->cache[l]->sid.length) == 0))
            {
                // This ladder should be compatible with the auth path
                for(uint16_t r=0; r<ladder_cache->cache[l]->rung_count; r++)
                {
                    RUNG *cache_rung = ladder_cache->cache[l]->rungs;

                    // Find a containing rung (getting the smallest span possible)
                    if((path->leaf_index >= cache_rung[r].left_index) && 
                       (path->leaf_index <= cache_rung[r].right_index)) {
                        // Find the smallest span possible (for best verification)
                        if(target == NULL) {
                            target = &cache_rung[r];
...
/**
 * Check that a ladder does not already exist with that rung set
 *
 * \param[in] max elements in include in the cache (LIFO)
 */
ldns_status ldns_dnssec_mtl_ladder_exists(LADDER* ladder)
{
    // Check if this ladder is already in cache and if so skip the extra validation work.
    for(uint16_t l=0; l<ladder_cache->max_entries; l++)
    {
        uint8_t rung_match = 1;
        if(ladder_cache->cache[l] != NULL)
        {
            if(ladder == ladder_cache->cache[l])
            {
                // This is the same ladder so return without any action
                return LDNS_STATUS_NULL;  						
            }

            if((ladder_cache->cache[l]->flags == ladder->flags) && 
                (ladder_cache->cache[l]->rung_count == ladder->rung_count) && 
                (ladder_cache->cache[l]->sid.length == ladder->sid.length) && 
                (memcmp(ladder_cache->cache[l]->sid.id, ladder->sid.id, ladder_cache->cache[l]->sid.length) == 0))
            {

                for(uint16_t r=0; r<ladder_cache->cache[l]->rung_count; r++)
                {
                    RUNG *cache_rung = ladder_cache->cache[l]->rungs;
                    RUNG *src_rung = ladder->rungs;
                    if((cache_rung[r].left_index != src_rung[r].left_index) || 
                        ...
                        (memcmp(cache_rung[r].hash, src_rung[r].hash, cache_rung[r].hash_length) != 0))
                    {
                        rung_match = 0;
                    }
                }

                // If ladders match, clean up and return OK
                if(rung_match) {
        ...
        )
{
    ldns_pkt * keypkt;
    ldns_rr * cur_key;
    size_t key_i; size_t key_j; size_t key_k;
    size_t sig_i; ldns_rr * cur_sig;
    ...
    ldns_buffer* rrset, unsigned char* key, char* algo)
{
    ...
    oqs_sig = OQS_SIG_new(algo);
    if (oqs_sig == NULL) {
        return LDNS_STATUS_ERR;
    }

    // Verify the signature
    if (OQS_SIG_verify(oqs_sig,  ldns_buffer_begin(rrset),
...
static ldns_status
ldns_verify_rrsig_mtl_raw(unsigned char* sig, size_t siglen,
    ldns_buffer* rrset, unsigned char* key, size_t keylen, uint8_t algo)
{
    ...
    memcpy(params->pk_root.key, key + params->pk_seed.length, params->pk_root.length);
    ...
    if(full_sig) {
        if(ldns_verify_rrsig_mtl_ladder(sig, siglen, key, keylen, algo) == LDNS_STATUS_CRYPTO_BOGUS) {
            LOG_ERROR("Full Signature cannot be verified on the ladder");
            return LDNS_STATUS_CRYPTO_BOGUS;
        }
    } 

    // Find the cached ladder that can be used to verify this signature
    rung = land_dnssec_mtl_ladder_find_rung(auth_path);

    if(rung != NULL) {
        if(mtl_hash_and_verify(mtl_ctx, ldns_buffer_begin(rrset), ldns_buffer_position(rrset), mtl_rand, auth_path, rung) != 0) {
            result = LDNS_STATUS_CRYPTO_BOGUS;
        }
    } else {
        result = LDNS_STATUS_CRYPTO_BOGUS;
...
/**
 * Verifies the signture on the mtl ladder if it is not in cache
 *
 * \param[in] sig Signed ladder buffer to verify
 * \param[in] siglen Length of the signed ladder
 * \param[in] key Public key signing the ladder
 * \param[in] keylen Length of the public key
 * \param[in] algo Signing algorithm
 * \return status LDNS_STATUS_OK if the data verifies. Error if not.
 */
ldns_status
ldns_verify_rrsig_mtl_ladder(unsigned char* sig, size_t siglen,
    ...
    params = calloc(1, sizeof(SPX_PARAMS));
    if(params == NULL) {
        LOG_ERROR("Unable to allocate scheme parameters");
        mtl_randomizer_free(mtl_rand);
        mtl_authpath_free(auth_path);
    ...
    if(full_sig) {
        ...
        ladder_buff_len = mtl_ladder_from_buffer((char*)buffer, buffer_len, seed.length, mtl_ctx->sid.length, &ladder);

        // Verify the signature on the ladder
        if(buffer_len > ladder_buff_len + 100) {        
            // Check if this ladder is already in cache and if so skip the extra validation work.
            ldns_status ladder_stat = ldns_dnssec_mtl_ladder_exists(ladder);
            if(ladder_stat == LDNS_STATUS_NULL) {
                // The exact ladder object was already in the ladder so return no free
                if(params) {
                    free(params);
                }		
                mtl_free(mtl_ctx);
            ...
            if(ladder_stat == LDNS_STATUS_EXISTS_ERR) {
                // The ladder copy was already in the cache so free this and return
                if(params) {
                    free(params);
                }		
                mtl_free(mtl_ctx);
            ...
            underlying_buffer_len = mtl_get_scheme_separated_buffer(mtl_ctx, ladder,
                            mtl_ctx->nodes.hash_size, &underlying_buffer, oid, oid_len);
            ...
            if (oqs_sig != NULL) {
                ...
                if(sig_length <= buffer_len) {
                    // Verify the signature
                    if (OQS_SIG_verify(oqs_sig, underlying_buffer, underlying_buffer_len,
                                        buffer, sig_length, key) != OQS_SUCCESS) {
                        result = LDNS_STATUS_CRYPTO_BOGUS;
                        if(ladder != NULL) {
                            mtl_ladder_free(ladder);
                            ladder = NULL;
                        ...
                        result = LDNS_STATUS_OK;
                        ldns_dnssec_mtl_ladder_cache_insert(ladder);
                    }
                } else {
                    result = LDNS_STATUS_CRYPTO_BOGUS;
                    if(ladder != NULL) {
...
/**
 * Verify the raw RRSIG data buffers with a specific key
 * @param sig: the signature on the data buffer
 * @param siglen: length in bytes of the signature
 * @param verify_buf: LDNS buffer for RRSIG data to verify
 * @param key: raw key data buffer
 * @param keylen: length of the key data buffer in bytes
 * @param algo: Algorithm identifier associated with the key
 */
ldns_status
ldns_verify_rrsig_buffers_raw(unsigned char* sig, size_t siglen,
                        ldns_buffer *verify_buf, unsigned char* key,
                        size_t keylen, uint8_t algo)
{
    ...
    case LDNS_DSA:
    case LDNS_DSA_NSEC3:
        return ldns_verify_rrsig_dsa_raw(sig, siglen, verify_buf,
                                         key, keylen);
        break;
#endif
    case LDNS_RSASHA1:
    case LDNS_RSASHA1_NSEC3:
        return ldns_verify_rrsig_rsasha1_raw(sig, siglen,
                                      verify_buf, key, keylen);
        break;
#ifdef USE_SHA2
    case LDNS_RSASHA256:
        return ldns_verify_rrsig_rsasha256_raw(sig, siglen,
                                        verify_buf, key, keylen);
        break;
    case LDNS_RSASHA512:
        return ldns_verify_rrsig_rsasha512_raw(sig, siglen,
                                        verify_buf, key, keylen);
        break;
#endif
#ifdef USE_GOST
