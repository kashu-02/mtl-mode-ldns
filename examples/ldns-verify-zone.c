		...
		bool detached_zonemd) 
{
	...
	ldns_dnssec_name* name = NULL;
	ldns_dnssec_rrsets *cur_rrset = NULL;		
#endif
	...
	if (!cur_key_rrset || !cur_key_rrset->rrs) {
...
#if defined(PQC_ALGO_SLH_DSA_MTL_SHA2) || defined (PQC_ALGO_SLH_DSA_MTL_SHAKE)
		// Check all the full signatures first
				...
				while(rr_sig != NULL) {
					if(
						...
						false
							...
							sigrdf = ldns_rr_rrsig_sig(rr_sig->rr);
							ldns_rdf2wire((uint8_t**)&mtl_sig, sigrdf, &mtl_sig_size);

							if(mtl_sig[0] == 1) {
								uint32_t ki;
								for(ki = 0; ki < ldns_rr_list_rr_count(keys); ki++) {
										...
										if(	
											...
											if (ldns_rdf2buffer_wire(key_buf, ldns_rr_rdf(key, 3)) == LDNS_STATUS_OK) {
												if( ldns_verify_rrsig_mtl_ladder(mtl_sig, mtl_sig_size, ldns_buffer_begin(key_buf), ldns_buffer_position(key_buf), algo) != LDNS_STATUS_OK) {
													// Nothing to do here because it could just be the wrong key, try the next one.
												}
											}
		...
		// Check all the signatures for validity
		cur_node = ldns_rbtree_first(dnssec_zone->names);
		while (cur_node != LDNS_RBTREE_NULL) {
...
#if defined(PQC_ALGO_SLH_DSA_MTL_SHA2) || defined (PQC_ALGO_SLH_DSA_MTL_SHAKE)
		/* There is no mechanism in this tool to fetch a full signature
		 * as if it is not in the zone file it is not available.  
		 * This is where that fetch would go if such a mechanism was
		 * to be added.
		 */
#endif
	}
...
#if defined(PQC_ALGO_SLH_DSA_MTL_SHA2) || defined (PQC_ALGO_SLH_DSA_MTL_SHAKE)
	ldns_dnssec_mtl_ladder_cache_init(128);
#endif
...
#if defined(PQC_ALGO_SLH_DSA_MTL_SHA2) || defined (PQC_ALGO_SLH_DSA_MTL_SHAKE)
	ldns_dnssec_mtl_ladder_cache_free();
#endif
	exit(result);
}
