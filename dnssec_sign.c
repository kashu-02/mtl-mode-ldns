...
ldns_sign_public_oqs(ldns_buffer *to_sign, oqs_key *key) {
    OQS_SIG *sig = NULL;
    ldns_rdf *sigdata_rdf = NULL;
	uint8_t *oqs_sig = NULL;
	size_t oqs_sig_len = 0;    

    // Sign the buffer with the scheme
    sig = OQS_SIG_new(key->alg_id);
    if (sig == NULL) {
        return NULL;
    }
                                        
    oqs_sig = malloc(sig->length_signature);
    OQS_SIG_sign(sig, oqs_sig, &oqs_sig_len,
        (unsigned char*)ldns_buffer_begin(to_sign),
        ldns_buffer_position(to_sign), key->sk);
    OQS_SIG_free(sig);

    sigdata_rdf = ldns_rdf_new_frm_data(LDNS_RDF_TYPE_B64, oqs_sig_len, oqs_sig);
    free(oqs_sig);
 ...
 */
ldns_rdf *
ldns_sign_public_mtl(ldns_buffer *to_sign, mtl_key *key) {
    ldns_rdf *sigdata_rdf;
    mtl_id id;

    memcpy(&id.sid, &key->mtl_ctx->sid, sizeof(SERIESID));
    if(mtl_hash_and_append(key->mtl_ctx, 
                        (unsigned char*)ldns_buffer_begin(to_sign),
                        ldns_buffer_position(to_sign), &id.leaf_id) != MTL_OK) {
		return NULL;
	}
...
ldns_dnssec_zone_update_mtl_rrsigs(ldns_rr_list *new_rrs, ldns_key_list *key_list) 
{
    ...
    size_t i = 0;
    mtl_id* mtl_key_id = NULL;
    ldns_rdf *sigrdf = NULL;
    size_t mtl_key_id_size = 0;
    ...
    for (i = 0; i < ldns_rr_list_rr_count(new_rrs); i++) {
        ...
        if(ldns_rr_get_type(rr_ptr) == LDNS_RR_TYPE_RRSIG) {
            uint16_t algo = (uint16_t)ldns_rdf2native_int8(ldns_rr_rrsig_algorithm(rr_ptr));
            ...
            switch(algo) {
                ...
                case LDNS_SIGN_SLH_DSA_MTL_SHAKE_128s:                
                    ...
                    for (key_count = 0; key_count < ldns_key_list_key_count(key_list); key_count++) {
                        current_key = ldns_key_list_key(key_list, key_count);
                        ...
                        if((priv_key != NULL) && 
                           (ldns_key_algorithm(current_key) == LDNS_SIGN_SLH_DSA_MTL_SHA2_128s ||
                            ...
                            if(memcmp(mtl_key_id->sid.id, mtl_ctx->sid.id, mtl_key_id->sid.length) == 0) { 
                                ldns_buffer* sign_buf = ldns_buffer_new(LDNS_MAX_PACKETLEN);
                                mtl_randomizer_and_authpath(mtl_ctx, mtl_key_id->leaf_id, &mtl_rand, &auth_path);

                                uint8_t* condensed_buffer = NULL;
                                size_t condensed_size = mtl_auth_path_to_buffer(mtl_rand, auth_path,
                                                mtl_ctx->nodes.hash_size, &condensed_buffer);
								mtl_authpath_free(auth_path);
								mtl_randomizer_free(mtl_rand);

                                ldns_buffer_write_u8(sign_buf, 0);
                                ldns_buffer_write(sign_buf, condensed_buffer, condensed_size);
                                free(condensed_buffer);
								...
								if(full_signature) 
								{
									// Set the flag for the full signature
									ldns_buffer_write_u8_at(sign_buf, 0, 1);

									// Get the signed ladder for the full signature
									uint8_t *ladder_buffer = NULL;
									uint32_t ladder_buffer_size = 0;
									uint8_t *underlying_buffer = NULL;      
									uint32_t underlying_buffer_len = 0;	                                                          
									LADDER *ladder = mtl_ladder(mtl_ctx);
									ladder_buffer_size =
										mtl_ladder_to_buffer(ladder, mtl_ctx->nodes.hash_size, &ladder_buffer);

									ldns_buffer_write(sign_buf, ladder_buffer, ladder_buffer_size);
									free(ladder_buffer);
									...
									underlying_buffer_len =
											mtl_get_scheme_separated_buffer(mtl_ctx, ladder,
															...
															&underlying_buffer, priv_key->oid, priv_key->oid_len);
									mtl_ladder_free(ladder);

									// Sign the ladder with the underlying scheme
									sig = OQS_SIG_new(priv_key->oqs_alg_id);
									if (sig == NULL) {
										ldns_buffer_free(sign_buf);										
									...
									ldns_buffer_write_u32(sign_buf, sig->length_signature);
									ladder_sig = malloc(sig->length_signature);
									if(ladder_sig == NULL) {
										ldns_buffer_free(sign_buf);
										OQS_SIG_free(sig);
										free(underlying_buffer);										
									...
									OQS_SIG_sign(sig, ladder_sig, &ladder_sig_len, underlying_buffer,
											underlying_buffer_len, priv_key->sk);
									OQS_SIG_free(sig);
									free(underlying_buffer);

									ldns_buffer_write(sign_buf, ladder_sig, ladder_sig_len);
									free(ladder_sig);
								} 

                                b64rdf = ldns_rdf_new_frm_data(LDNS_RDF_TYPE_B64, ldns_buffer_position(sign_buf), ldns_buffer_begin(sign_buf));
								if(b64rdf == NULL) {
									ldns_buffer_free(sign_buf);
									return LDNS_STATUS_MEM_ERR;
								}
                                ldns_rr_rrsig_set_sig(rr_ptr, b64rdf);
                                ldns_buffer_free(sign_buf);
                            }
                        }
                    }
